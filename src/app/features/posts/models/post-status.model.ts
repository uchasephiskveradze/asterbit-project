...
export const MY_POSTS_TAB_LABELS: Record<MyPostsTab, string> = {
  'under-review': 'posts.myPosts.tabs.underReview',
  approved: 'posts.myPosts.tabs.approved',
  rejected: 'posts.myPosts.tabs.rejected',
};
...
export const POST_STATUS_LABEL_KEYS: Record<PostStatus, string> = {
  [POST_STATUS.pending]: 'posts.status.underReview',
  [POST_STATUS.approved]: 'posts.status.approved',
  [POST_STATUS.rejected]: 'posts.status.rejected',
};

export const PENDING_REASON_LABEL_KEYS: Record<PostPendingReason, string> = {
  new: 'posts.status.newSubmission',
  edited: 'posts.status.editedSubmission',
};

export const PENDING_REASON_DEFAULT_KEY = 'posts.status.pendingReview';
...
export function getPendingReasonLabelKey(reason: string | undefined): string {
  if (reason && isPendingReason(reason)) {
    return PENDING_REASON_LABEL_KEYS[reason];
  }

  return PENDING_REASON_DEFAULT_KEY;
}
